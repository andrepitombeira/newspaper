...
public class EditorRemoverSecaoServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if(user != null && user instanceof Editor){
                ...
                if(editor.getSecao(id) != null){
                    SecaoDAO dao = new SecaoDAO();
                    dao.excluir(id);
                    editor.removerSecao(id);
                    session.setAttribute("sucesso", "Secao removida com sucesso.");
                    response.sendRedirect(request.getContextPath() + "/editorListarSecoes.jsp");
                }else{
                    session.setAttribute("error", "Voce nao tem permissao para apagar essa secao");
                    response.sendRedirect(request.getContextPath() + "/editorListarSecoes.jsp");
                }
            }else{
