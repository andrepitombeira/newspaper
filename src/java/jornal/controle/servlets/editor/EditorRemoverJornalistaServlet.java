...
public class EditorRemoverJornalistaServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if(user != null && user instanceof Editor){
                ...
                if(editor.getJornalista(id) != null){
                    JornalistaDAO dao = new JornalistaDAO();
                    dao.excluir(id);
                    editor.removerJornalista(id);
                    session.setAttribute("sucesso", "Jornalista removido com sucesso.");
                    response.sendRedirect(request.getContextPath() + "/editorListarJornalistas.jsp");
                }else{
                    session.setAttribute("error", "Voce nao tem permissao para apagar esse jornalista");
                    response.sendRedirect(request.getContextPath() + "/editorListarJornalistas.jsp");
                }
            }else{
